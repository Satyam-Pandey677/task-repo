...
const HrDashboard = () => {
  ...
  useEffect(() => {
    axios.get('/api/employee/attendance', { headers: getHeaders() })
  ...
  const stats = [
    { label: 'Total employees', value: String(total), detail: total ? 'Live roster' : 'No data', tone: 'bg-emerald-50 text-emerald-700', icon: '👥' },
    { label: 'Present today', value: String(present), detail: `${rate}% attendance`, tone: 'bg-sky-50 text-sky-700', icon: '✓' },
    { label: 'On leave', value: String(leave), detail: 'Team status', tone: 'bg-amber-50 text-amber-700', icon: '◷' },
    { label: 'Late arrivals', value: String(late), detail: 'Need review', tone: 'bg-rose-50 text-rose-700', icon: '!' },
  ];

  const breakdown = [
    { label: 'Present', value: total ? Math.round((present / total) * 100) : 0, color: 'bg-emerald-500' },
    { label: 'Late', value: total ? Math.round((late / total) * 100) : 0, color: 'bg-amber-500' },
    { label: 'On leave', value: total ? Math.round((leave / total) * 100) : 0, color: 'bg-slate-400' },
  ];
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_1fr]">
        <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{loading ? 'Loading...' : `${rate}% average`}</span>
          </div>
        ...
        <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="text-sm font-semibold text-emerald-600">{present} present</span>
          </div>
          <div className="mt-5 divide-y divide-slate-100">
            {records.length ? records.slice(0, 4).map((person) => (
              <div key={person.name} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-slate-500">{person.department}</p>
                </div>
                <div className="text-right">
                  <p className="flex items-center justify-end gap-1.5 text-xs font-semibold text-slate-700">
                    <span className={`h-2 w-2 rounded-full ${person.status === 'Present' ? 'bg-emerald-500' : person.status === 'Late' ? 'bg-amber-500' : 'bg-slate-400'}`} />
                    {person.status}
                  </p>
                  <p className="mt-1 text-[11px] text-slate-400">{person.checkIn !== '--' ? person.checkIn : 'Today'}</p>
                </div>
              </div>
            )) : (
